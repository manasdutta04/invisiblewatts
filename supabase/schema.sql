...
CREATE TABLE IF NOT EXISTS public.upload_events (
  ...
  event_date   DATE,
  device_type  TEXT NOT NULL DEFAULT 'unknown', -- 'phone', 'laptop', 'tablet', 'desktop', etc
  entry_count  INTEGER,
  error_message TEXT,
  metadata     JSONB -- for storing additional info like file size, validation errors, etc
);
...
CREATE POLICY "Users can access own upload events"
